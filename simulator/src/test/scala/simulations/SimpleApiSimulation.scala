...
class SimpleApiSimulation extends Simulation:
  ...
  val scn: ScenarioBuilder = scenario("Log upload performance test")
    .feed(feeder)
    .exec(
      http("Post logs")
  ...
  setUp(scn.inject(constantUsersPerSec(300) during 30.seconds)).protocols(httpProtocol)
